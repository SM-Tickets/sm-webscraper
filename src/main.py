...
class AxsSeriesWebscraper(Webscraper):
    ...
    def get_titles(cls, url_to_response: dict[str, Response], log_callback=None):
        ...
        for url, response in url_to_response.items():
            title_tag = response.find("h1", lambda elem: elem.has_class("css-tay3x2-SeriesTitle")) or response.find( "h1", lambda elem: elem.has_class("styles__SeriesTitle-sc-d3a1a166-1"))
